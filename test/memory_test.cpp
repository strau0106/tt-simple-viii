...
TEST_F(Memory, TuringRequirement1321) {
    ...
    ASSERT_EQ(memory_dut->rootp->memory__DOT__cells.m_storage[ARBITRARY_WRITE_LOCATION<<1], ARBITRARY_DATA); // << 1 because of data_word_selector

    #define ARBIRARY_READ_LOCATION 0b111010001

    // data_word_selector is still 0

    memory_dut->rootp->memory__DOT__cells.m_storage[ARBIRARY_READ_LOCATION<<1] = ARBITRARY_DATA;
